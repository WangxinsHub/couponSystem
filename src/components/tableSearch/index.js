 ...
 * const searchMenu = {      
      ...
      searchCallBack: this.handleSearch, // 查询的回调函数
        ...
        item.map((value, index) => {          
          ...
          {            
            value.map((key, _index) => {
              ...
              if(_index === value.length-1 && index===item.length-1 && item[item.length-1].length===1) {
                isAddCol=true;
              }
              switch(key.type) {
                case 'input':      
                  ...
                  return (<Col {...colLayout} key={_index}>
                      <FormItem label={label} {...formItemLayout}>
                        {getFieldDecorator(id, {
                          ...
                          <Select placeholder={placeholder} key={id} showSearch optionFilterProp={'children'}>
